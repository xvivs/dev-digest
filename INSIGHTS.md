...
- **The shared DB has drifted further since 2026-09-19: `agent_runs.cost_source` now exists too** (the earlier Open Questions entry recorded it as missing), along with the `agent_runs_pr_ran_at_idx` index — i.e. everything migration `0010` wants to add is already present, which is exactly why it fails. The drift is one-directional and keeps growing as other worktrees migrate the same container. _(2026-09-20)_
